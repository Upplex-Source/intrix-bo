<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.brands' ) }}</h3>
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.brand.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'brand.title' ) ] ),
        'id' => 'title',
        'title' => __( 'brand.title' ),
    ],
    // [
    //     'type' => 'default',
    //     'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'brand.number_of_product' ) ] ),
    //     'id' => 'number_of_product',
    //     'title' => __( 'brand.number_of_product' ),
    // ],
    // [
    //     'type' => 'input',
    //     'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'brand.stock_quantity' ) ] ),
    //     'id' => 'stock_quantity',
    //     'title' => __( 'brand.stock_quantity' ),
    // ],
    // [
    //     'type' => 'input',
    //     'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'brand.stock_worth' ) ] ),
    //     'id' => 'stock_worth',
    //     'title' => __( 'brand.stock_worth' ),
    // ],
    [
        'type' => 'select',
        'options' => $data['status'],
...
<x-data-tables id="brand_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = @json( $data['status'] ),
    ...
    dt_table_config = {
        ...
        columnDefs: [
            ...
            {
                targets: parseInt( '{{ Helper::columnIndex( $columns, "parent_brand" ) }}' ),
                width: '10%',
                render: function( data, type, row, meta ) {
                    return data ? data : '-' ;
                },
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.brand.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.brand.updateBrandStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
