<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.guide_countries' ) }}</h3>
        </div><!-- .nk-block-head-content -->
        @can( 'add guide_countries' )
        <div class="nk-block-head-content">
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.guide_country.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'default',
        'id' => 'image',
        'title' => __( 'guide_country.image' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'guide_country.name' ) ] ),
        'id' => 'name',
        'title' => __( 'guide_country.name' ),
    ],
    [
        'type' => 'default',
        'id' => 'states',
        'title' => __( 'guide_country.states' ),
    ],
    [
        'type' => 'default',
        'id' => 'branches',
        'title' => __( 'guide_country.branches' ),
    ],
    [
        'type' => 'select',
        'options' => $data['status'],
...
var statusMapper = @json( $data['status'] ),
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit guide_countries', 'delete guide_countries' ] )
                    let edit, status = '';

                    @can( 'edit guide_countries' )
                    edit = '<li class="dt-edit" data-id="' + row['encrypted_id'] + '"><a href="#"><em class="icon ni ni-edit"></em><span>{{ __( 'template.edit' ) }}</span></a></li>';
                    @endcan

                    @can( 'delete guide_countries' )
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.guide_country.edit' ) }}?id=' + $( this ).data( 'id' );
        } );
