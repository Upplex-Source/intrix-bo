<?php
$user_checkin_edit = 'user_checkin_edit';
?>

<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.edit_x', [ 'title' => Str::singular( __( 'template.user_checkins' ) ) ] ) }}</h3>
        ...
        <div class="row">
            <div class="col-md-12 col-lg-6">
                ...
                <div class="text-end">
                    ...
                    &nbsp;
                    <button id="{{ $user_checkin_edit }}_submit" type="button" class="btn btn-primary">{{ __( 'template.save_changes' ) }}</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( fe + '_submit' ).click( function() {
            ...
            $.ajax( {
                url: '{{ route( 'admin.user_checkin.updateUserCheckin' ) }}',
                type: 'POST',
                data: formData,
                processData: false,
                contentType:   false,
        ...
        function getUserCheckin() {
            ...
            $.ajax( {
                url: '{{ route( 'admin.user_checkin.oneUserCheckin' ) }}',
                type: 'POST',
                data: {
                    'id': '{{ request( 'id' ) }}',
                    '_token': '{{ csrf_token() }}'
