<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.product_add_ons' ) }}</h3>
        </div><!-- .nk-block-head-content -->
        @can( 'add product_add_ons' )
        <div class="nk-block-head-content">
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.product_add_on.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'default',
        'id' => 'image',
        'title' => __( 'product_add_on.image' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'product_add_on.sku' ) ] ),
        'id' => 'sku',
        'title' => __( 'product_add_on.sku' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'product_add_on.code' ) ] ),
        'id' => 'code',
        'title' => __( 'product_add_on.code' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'product_add_on.title' ) ] ),
        'id' => 'title',
...
var statusMapper = @json( $data['status'] ),
    ...
    dt_table_config = {
        ...
        ajax: {
            url: '{{ route( 'admin.product_add_on.allProductAddOns' ) }}',
            data: {
                '_token': '{{ csrf_token() }}',
            },
            dataSrc: 'product_add_ons',
        },
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit product_add_ons', 'delete product_add_ons' ] )
                    let edit, status = '';

                    @can( 'edit product_add_ons' )
                    edit = '<li class="dt-edit" data-id="' + row['encrypted_id'] + '"><a href="#"><em class="icon ni ni-edit"></em><span>{{ __( 'template.edit' ) }}</span></a></li>';
                    @endcan

                    @can( 'delete product_add_ons' )
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.product_add_on.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.product_add_on.updateProductAddOnStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
