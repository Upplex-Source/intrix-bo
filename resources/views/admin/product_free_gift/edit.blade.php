...
<?php
$product_free_gift_edit = 'product_free_gift_edit';
?>
        ...
        <div class="row">
            <div class="col-md-12 col-lg-12">
                ...
                <div class="mb-3">
                    ...
                    <div class="dropzone mb-3" id="{{ $product_free_gift_edit }}_image" style="min-height: 0px;">
                ...
                <div class="mb-3 row">
                    <label for="{{ $product_free_gift_edit }}_title" class="col-sm-5 form-label">{{ __( 'product_free_gift.title' ) }}</label>
                    <div class="col-sm-7">
                        <input type="text" class="form-control" id="{{ $product_free_gift_edit }}_title">
                        <div class="invalid-feedback"></div>
                    </div>
                </div>

                <div class="mb-3 row">
                    <label for="{{ $product_free_gift_edit }}_code" class="col-sm-5 form-label">{{ __( 'product_free_gift.code' ) }}</label>
                    <div class="col-sm-7">
                        <input type="text" class="form-control" id="{{ $product_free_gift_edit }}_code">
                        <div class="invalid-feedback"></div>
                    </div>
                </div>
                
                <div class="mb-3 row">
                    <label for="{{ $product_free_gift_edit }}_sku" class="col-sm-5 form-label">{{ __( 'product_free_gift.sku' ) }}</label>
                    <div class="col-sm-7">
                        <input type="text" class="form-control" id="{{ $product_free_gift_edit }}_sku">
                        <div class="invalid-feedback"></div>
                    </div>
                </div>

                <div class="mb-3 row">
                    <label for="{{ $product_free_gift_edit }}_description" class="col-sm-5 form-label">{{ __( 'product_free_gift.description' ) }}</label>
                    <div class="col-sm-7">
                        <textarea class="form-control" name="{{ $product_free_gift_edit }}_description" id="{{ $product_free_gift_edit }}_description" rows="5"></textarea>
                        <div class="invalid-feedback"></div>
                    </div>
                </div>

                <div class="mb-3 row">
                    <label for="{{ $product_free_gift_edit }}_price" class="col-sm-5 form-label">{{ __( 'product_free_gift.price' ) }}</label>
                    <div class="col-sm-7">
                        <input type="number" class="form-control" id="{{ $product_free_gift_edit }}_price">
                        <div class="invalid-feedback"></div>
                    </div>
                </div>
                ...
                <div class="mb-3 row">
                    <label for="{{ $product_free_gift_edit }}_product" class="col-sm-5 form-label">{{ __( 'product_free_gift.product' ) }}</label>
                    <div class="col-sm-7">
                        <select class="form-select" id="{{ $product_free_gift_edit }}_product" data-placeholder="{{ __( 'datatables.select_x', [ 'title' => __( 'product_free_gift.product' ) ] ) }}" multiple="multiple">
                        </select>
                        <div class="invalid-feedback"></div>
                    </div>
                </div>
                ...
                <div class="text-end">
                    <button id="{{ $product_free_gift_edit }}_cancel" type="button" class="btn btn-outline-secondary">{{ __( 'template.cancel' ) }}</button>
                    &nbsp;
                    <button id="{{ $product_free_gift_edit }}_submit" type="button" class="btn btn-primary">{{ __( 'template.save_changes' ) }}</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( fe + '_submit' ).click( function() {
            ...
            $.ajax( {
                url: '{{ route( 'admin.product_free_gift.updateProductFreeGift' ) }}',
                type: 'POST',
                data: formData,
                processData: false,
                contentType:   false,
        ...
        function getProductFreeGift() {
            ...
            $.ajax( {
                ...
                url: '{{ route( 'admin.product_free_gift.oneProductFreeGift' ) }}',
                type: 'POST',
                data: {
                    'id': '{{ request( 'id' ) }}',
                    '_token': '{{ csrf_token() }}'
                ...
                success: function( response ) {
                    ...
                    response.free_gift_products.forEach((product, index) => {
