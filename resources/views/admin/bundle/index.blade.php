<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.bundles' ) }}</h3>
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.bundle.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'default',
        'id' => 'image',
        'title' => __( 'bundle.image' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'bundle.title' ) ] ),
        'id' => 'title',
    ...
    //     'title' => __( 'bundle.number_of_product' ),
    // ],
    // [
    //     'type' => 'input',
    //     'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'bundle.stock_quantity' ) ] ),
    //     'id' => 'stock_quantity',
    //     'title' => __( 'bundle.stock_quantity' ),
    // ],
    // [
    //     'type' => 'input',
    //     'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'bundle.stock_worth' ) ] ),
    //     'id' => 'stock_worth',
    //     'title' => __( 'bundle.stock_worth' ),
    // ],
    [
        'type' => 'select',
        'options' => $data['status'],
...
var statusMapper = @json( $data['status'] ),
    ...
    dt_table_config = {
        ...
        ajax: {
            url: '{{ route( 'admin.bundle.allBundles' ) }}',
            data: {
                '_token': '{{ csrf_token() }}',
            },
            dataSrc: 'bundles',
        ...
        columnDefs: [
            ...
            {
                targets: parseInt( '{{ Helper::columnIndex( $columns, "parent_bundle" ) }}' ),
                width: '10%',
                render: function( data, type, row, meta ) {
                    return data ? data : '-' ;
                },
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit bundles', 'delete bundles' ] )
                    let edit, status = '';
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.bundle.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.bundle.updateBundleStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
