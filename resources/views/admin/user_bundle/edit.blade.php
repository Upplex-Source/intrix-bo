<?php
$user_bundle_edit = 'user_bundle_edit';
?>
    ...
    <div class="card-inner">
        <div class="row">
            <div class="col-md-12 col-lg-12">
                ...
                <div class="mb-3 row">
                    <label for="{{ $user_bundle_edit }}_user" class="col-sm-4 col-form-label">{{ __( 'user_bundle.user' ) }}</label>
                    <div class="col-sm-6">
                        <select class="form-select" id="{{ $user_bundle_edit }}_user" data-placeholder="{{ __( 'datatables.select_x', [ 'title' => __( 'user_bundle.user' ) ] ) }}" multiple="multiple">
                        </select>
                ...
                <div class="mb-3 row">
                    <label for="{{ $user_bundle_edit }}_product_bundle" class="col-sm-4 col-form-label">{{ __( 'user_bundle.product_bundle' ) }}</label>
                    <div class="col-sm-6">
                        <select class="form-select" id="{{ $user_bundle_edit }}_product_bundle" data-placeholder="{{ __( 'datatables.select_x', [ 'title' => __( 'user_bundle.product_bundle' ) ] ) }}">
                        </select>
                        <div class="invalid-feedback"></div>
                    </div>
                </div>
                ...
                <div class="text-end">
                    <button id="{{ $user_bundle_edit }}_cancel" type="button" class="btn btn-outline-secondary">{{ __( 'template.cancel' ) }}</button>
                    &nbsp;
                    <button id="{{ $user_bundle_edit }}_submit" type="button" class="btn btn-primary">{{ __( 'template.save_changes' ) }}</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( fc + '_submit' ).click( function() {
            ...
            $.ajax( {
                url: '{{ route( 'admin.user_bundle.updateUserBundle' ) }}',
                type: 'POST',
                data: formData,
                processData: false,
                contentType:   false,
        ...
        let productBundleSelect2 = $( fc + '_product_bundle' ).select2( {
            theme: 'bootstrap-5',
            width: $( this ).data( 'width' ) ? $( this ).data( 'width' ) : $( this ).hasClass( 'w-100' ) ? '100%' : 'style',
            placeholder: $( this ).data( 'placeholder' ),
            closeOnSelect: false,
