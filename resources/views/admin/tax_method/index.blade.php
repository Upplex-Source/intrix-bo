<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.tax_methods' ) }}</h3>
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.tax_method.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'default',
        'id' => 'image',
        'title' => __( 'tax_method.image' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'tax_method.title' ) ] ),
        'id' => 'title',
        'title' => __( 'tax_method.title' ),
    ],
    [
        'type' => 'default',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'tax_method.tax_percentage' ) ] ),
        'id' => 'tax_percentage',
        'title' => __( 'tax_method.tax_percentage' ),
    ],
    // [
    //     'type' => 'input',
    //     'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'tax_method.stock_quantity' ) ] ),
    //     'id' => 'stock_quantity',
    //     'title' => __( 'tax_method.stock_quantity' ),
    // ],
    // [
    //     'type' => 'input',
    //     'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'tax_method.stock_worth' ) ] ),
    //     'id' => 'stock_worth',
    //     'title' => __( 'tax_method.stock_worth' ),
    // ],
    [
        'type' => 'select',
        'options' => $data['status'],
...
var statusMapper = @json( $data['status'] ),
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit tax_methods', 'delete tax_methods' ] )
                    let edit, status = '';

                    @can( 'edit tax_methods' )
                    edit = '<li class="dt-edit" data-id="' + row['encrypted_id'] + '"><a href="#"><em class="icon ni ni-edit"></em><span>{{ __( 'template.edit' ) }}</span></a></li>';
                    @endcan

                    @can( 'delete tax_methods' )
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.tax_method.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.tax_method.updateTaxMethodStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
