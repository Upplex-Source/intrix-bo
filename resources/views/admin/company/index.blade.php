<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.companies' ) }}</h3>
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.company.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'company.name' ) ] ),
        'id' => 'name',
...
<x-data-tables id="company_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = @json( $data['status'] ),
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.company.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.company.updateCompanyStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
