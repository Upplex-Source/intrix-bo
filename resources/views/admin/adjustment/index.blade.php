<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.adjustments' ) }}</h3>
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.adjustment.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
<x-data-tables id="adjustment_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = @json( $data['status'] ),
    dt_table,
    dt_table_name = '#adjustment_table',
    dt_table_config = {
        language: {
        ...
        ajax: {
            url: '{{ route( 'admin.adjustment.allAdjustments' ) }}',
            data: {
                '_token': '{{ csrf_token() }}',
            },
            dataSrc: 'adjustments',
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit adjustments', 'delete adjustments' ] )
                    let edit, status = '';
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.adjustment.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.adjustment.updateAdjustmentStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
