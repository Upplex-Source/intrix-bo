<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.outlets' ) }}</h3>
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.outlet.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'default',
        'id' => 'image',
        'title' => __( 'outlet.image' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'outlet.title' ) ] ),
        'id' => 'title',
        'title' => __( 'outlet.title' ),
    ],
    // [
    //     'type' => 'default',
    //     'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'outlet.number_of_product' ) ] ),
    //     'id' => 'number_of_product',
    //     'title' => __( 'outlet.number_of_product' ),
    // ],
    // [
    //     'type' => 'input',
    //     'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'outlet.stock_quantity' ) ] ),
    //     'id' => 'stock_quantity',
    //     'title' => __( 'outlet.stock_quantity' ),
    // ],
    // [
    //     'type' => 'input',
    //     'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'outlet.stock_worth' ) ] ),
    //     'id' => 'stock_worth',
    //     'title' => __( 'outlet.stock_worth' ),
    // ],
    [
        'type' => 'select',
        'options' => $data['status'],
...
<x-data-tables id="outlet_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = @json( $data['status'] ),
    ...
    dt_table_config = {
        ...
        ajax: {
            url: '{{ route( 'admin.outlet.allOutlets' ) }}',
            data: {
                '_token': '{{ csrf_token() }}',
            },
            dataSrc: 'outlets',
        ...
        columnDefs: [
            ...
            {
                targets: parseInt( '{{ Helper::columnIndex( $columns, "parent_outlet" ) }}' ),
                width: '10%',
                render: function( data, type, row, meta ) {
                    return data ? data : '-' ;
                },
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit outlets', 'delete outlets' ] )
                    let edit, status = '';
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.outlet.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.outlet.updateOutletStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
