<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.sales_orders' ) }}</h3>
        </div><!-- .nk-block-head-content -->
        @can( 'add sales_orders' )
        <div class="nk-block-head-content">
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.sales_order.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'sales_order.quotation' ) ] ),
        'id' => 'quotation',
        'title' => __( 'sales_order.quotation' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'sales_order.reference' ) ] ),
        'id' => 'reference',
        'title' => __( 'sales_order.reference' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'sales_order.salesman' ) ] ),
        'id' => 'salesman',
        'title' => __( 'sales_order.salesman' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'sales_order.customer' ) ] ),
        'id' => 'customer',
        'title' => __( 'sales_order.customer' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'sales_order.supplier' ) ] ),
        'id' => 'supplier',
        'title' => __( 'sales_order.supplier' ),
    ],
    [
        'type' => 'default',
        'id' => 'final_amount',
        'title' => __( 'sales_order.final_amount' ),
    ],
    [
        'type' => 'select',
        'options' => $data['status'],
...
<x-data-tables id="sales_order_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = @json( $data['status'] ),
    dt_table,
    dt_table_name = '#sales_order_table',
    dt_table_config = {
        language: {
        ...
        ajax: {
            url: '{{ route( 'admin.sales_order.allSalesOrders' ) }}',
            data: {
                '_token': '{{ csrf_token() }}',
            },
            dataSrc: 'sales_orders',
        },
        ...
        columnDefs: [
            ...
            {
                targets: parseInt( '{{ Helper::columnIndex( $columns, "quotation" ) }}' ),
                width: '10%',
                render: function( data, type, row, meta ) {
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit sales_orders', 'delete sales_orders' ] )
                    let edit, status = '';
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.sales_order.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.sales_order.updateSalesOrderStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
        ...
        $( document ).on( 'click', '.dt-convert', function() {
            ...
            $.ajax( {
                url: '{{ route( 'admin.sales_order.convertInvoice' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
                    '_token': '{{ csrf_token() }}'
